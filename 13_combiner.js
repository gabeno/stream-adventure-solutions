...
module.exports = function() {
  ...
  function write(line) {
    if (line.length === 0) return; // no data
    var row = JSON.parse(line); // js object

    if (row.type === 'genre') {
      if (collection) {
        // pass on existing collection to stream
        this.queue(JSON.stringify(collection) + '\n');
      }
      // create new collection
      // acts as a flag set to true when assigned an object
      collection = { name: row.name, books: [] };
    }
    else if (row.type === 'book') {
      // add book to collection
      collection.books.push(row.name);
    }
  }
